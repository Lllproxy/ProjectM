...
public class WebInfoController {
    ...
    @DeleteMapping("/delete/{type}/{w_id}")
    ...
    public Result delete(@PathVariable Integer type,@PathVariable String  w_id) {
        String optype;
        try {
            if (type.equals(1)){
                optype="禁用";
                //禁用服务
                //1.查询该服务
                WebInfo web=webInfoService.findById(w_id);
                //2.设置禁用
                web.setwStatus(2);
                webInfoService.update(web);
